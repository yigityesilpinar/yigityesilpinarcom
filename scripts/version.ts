...
const packageJSONOnly = cliArgs.some((arg) => arg.toLowerCase().includes('packagejsononly'))

let tag = '0.0.1'
const versionFilePath = path.join(OUTPUT_DIR, 'version.txt')
const packagejsonPath = path.join(PROJECT_ROOT_DIR, 'package.json')
// find the last tag
const gitDescribeCommand = spawn('sh', ['-c', 'git tag --sort=committerdate | tail -1'], {})
...
gitDescribeCommand.on('exit', () => {
  ...
  if (packageJSONOnly) {
    ...
    fs.writeFileSync(packagejsonPath, file.replace(/"version": ".*"/, `"version": "${tag}"`))
  } else {
